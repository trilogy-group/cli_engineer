...
use crate::event_bus::{EventBus, Event, EventEmitter, Metrics};
...
impl EnhancedUI {
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        println!("{}", "CLI Engineer - Autonomous Coding Agent".bright_white().bold());
        println!("{}", "=".repeat(80).bright_blue());
        println!();
                    ...
                    *last_metrics.write().await = metrics.clone();
                    ...
                    if let Some(bar) = &metrics_bar {
                        ...
                        let status = format!(
                            ...
                            format!("💾 Context: {:.0}%", metrics.current_context_usage).bright_blue(),
                        );
    ...
    pub fn finish(&mut self) {
        ...
        // Show final summary
        let metrics = executor::block_on(async {
        ...
        println!("⏱️  Duration: {}:{:02}", elapsed.as_secs() / 60, elapsed.as_secs() % 60);
        println!("✅ Tasks Completed: {}", metrics.tasks_completed.to_string().bright_green());
        println!("❌ Tasks Failed: {}", metrics.tasks_failed.to_string().bright_red());
        println!("🤖 Total API Calls: {}", metrics.total_api_calls.to_string().bright_cyan());
        println!("🪙  Total Tokens: {}", metrics.total_tokens.to_string().bright_cyan());
        println!("💰 Total Cost: ${:.4}", metrics.total_cost.to_string().bright_yellow());
        println!("📝 Artifacts Created: {}", metrics.artifacts_created.to_string().bright_magenta());
        println!();
    ...
    async fn handle_event(
        ...
        match event {
            ...
            Event::ArtifactCreated { name, artifact_type, .. } => {
                if let Some(pb) = main_progress {
                    pb.set_message(format!("📄 Created {} ({})", name.bright_green(), artifact_type));
                }
            }
